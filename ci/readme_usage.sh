#!/usr/bin/env bash
#
# zozz — README.md's Usage snippet, extracted from examples/usage.zig.
#
...
imports = [
    line for line in text.splitlines() if line.startswith('const zozz = @import(')
]
if len(imports) != 1:
    sys.exit("%s: expected exactly one `const zozz = @import(...)` line" % source)
...
# The marker line leaves a trailing indent behind on the line that opened it.
print("```zig")
print(imports[0])
print()
print("\n".join(body).strip("\n"))
