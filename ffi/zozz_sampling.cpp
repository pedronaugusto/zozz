...
ZozzResult zozzSamplingContextCreate(int max_tracks,
  ...
  *out = nullptr;
  ...
  ZozzSamplingContext* context = ozz::New<ZozzSamplingContext>();
  if (context == nullptr) return ZOZZ_ERR_OUT_OF_MEMORY;
...
ZozzResult zozzSample(const ZozzAnimation* animation,
  ...
  if (ratio != ratio) return ZOZZ_ERR_INVALID_ARGUMENT;  // NaN

  const int tracks = animation->impl.num_tracks();
  if (out->num_joints < tracks) return ZOZZ_ERR_BUFFER_TOO_SMALL;
  if (context->impl.max_tracks() < tracks) return ZOZZ_ERR_BUFFER_TOO_SMALL;
  ...
  if (!job.Validate()) return ZOZZ_ERR_JOB_INVALID;
  if (!job.Run()) return ZOZZ_ERR_JOB_INVALID;
  return ZOZZ_OK;
}
...
ZozzResult zozzLocalToModel(const ZozzSkeleton* skeleton,
  ...
  if (!IsAligned16(out)) return ZOZZ_ERR_INVALID_ARGUMENT;
  if (root != nullptr && !IsAligned16(root)) return ZOZZ_ERR_INVALID_ARGUMENT;

  const int joints = skeleton->impl.num_joints();
  if (locals->num_joints != joints) return ZOZZ_ERR_SKELETON_MISMATCH;
