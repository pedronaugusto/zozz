...
// consumer (the Zig wrapper in ../src, or a plain C host). They are
// deliberately free of C++: opaque handles, POD structs with fixed layout, and
// a flat error enum. No exceptions cross this boundary; ozz is compiled
// -fno-exceptions.
//
// Include ../ffi/zozz.h — the umbrella — and you have the whole surface. The
// parts are split by concern rather than by convenience, and each one stands
// on its own given this file, but one include is the contract.
//
...
//
// The declared surface does NOT depend on the build options. Nothing here
// changes shape with a `-D` flag, which is what lets ../src/abi_check.zig
// compare this header against the Zig externs in a single configuration and
// have the answer mean something in every other one.
//===----------------------------------------------------------------------===//
