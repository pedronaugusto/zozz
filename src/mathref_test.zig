...
//! constants below, each carrying what makes it necessary, and every row
//! states one. A deviation no constant covers is a finding, not a number to
//! raise.
...
const Case = struct {
    ...
    impl: Impl,
    fill: Fill = &any,
    /// Named on every row rather than defaulted: an unconsidered tolerance and
    /// a proven bit-exact one would otherwise be the same text. NORMALIZESAFE2
    /// and NORMALIZESAFE3 claimed exactness by saying nothing, held on the
    /// x86-64 that wrote them, and deviated by an ulp on aarch64 and on
    /// baseline x86-64 the first time either ran.
    tol: f32,
...
const cases = [_]Case{
    ...
    .{ .name = "DIVX", .impl = adapt(sf.divX), .fill = &nonZero, .tol = exact },
    .{ .name = "HADD2", .impl = adapt(sf.hAdd2), .tol = exact, .defined = lane_x },
    .{ .name = "HADD3", .impl = adapt(sf.hAdd3), .tol = rounding, .defined = lane_x },
    .{ .name = "HADD4", .impl = adapt(sf.hAdd4), .tol = rounding, .defined = lane_x },
    .{ .name = "SQRT", .impl = adapt(sf.sqrt), .fill = &positive, .tol = exact },
    .{ .name = "SQRTX", .impl = adapt(sf.sqrtX), .fill = &positive, .tol = exact },
    .{ .name = "ABS", .impl = adapt(sf.abs), .tol = exact },
    .{ .name = "MIN0", .impl = adapt(sf.min0), .tol = exact },
    .{ .name = "MAX0", .impl = adapt(sf.max0), .tol = exact },
    .{ .name = "FROMINT", .impl = adapt(sf.fromInt), .fill = &ints, .tol = exact },
    .{ .name = "SELECT", .impl = adapt(sf.select), .fill = &mask, .tol = exact },
    ...
    .{ .name = "NORMALIZESAFE2", .impl = adapt(sf.normalizeSafe2), .fill = safeFill(2), .tol = rounding, .defined = &.{2} },
    .{ .name = "NORMALIZESAFE3", .impl = adapt(sf.normalizeSafe3), .fill = safeFill(3), .tol = rounding, .defined = &.{3} },
    .{ .name = "NORMALIZESAFE4", .impl = adapt(sf.normalizeSafe4), .fill = safeFill(4), .tol = rounding },
    .{ .name = "ISNORMALIZED2", .impl = adapt(sf.isNormalized2), .fill = maybeUnitFill(2), .tol = exact },
    .{ .name = "ISNORMALIZED3", .impl = adapt(sf.isNormalized3), .fill = maybeUnitFill(3), .tol = exact },
    .{ .name = "ISNORMALIZED4", .impl = adapt(sf.isNormalized4), .fill = maybeUnitFill(4), .tol = exact },
    ...
    .{ .name = "ISNORMALIZEDEST2", .impl = adapt(sf.isNormalizedEst2), .fill = maybeUnitFill(2), .tol = exact },
    .{ .name = "ISNORMALIZEDEST3", .impl = adapt(sf.isNormalizedEst3), .fill = maybeUnitFill(3), .tol = exact },
    .{ .name = "ISNORMALIZEDEST4", .impl = adapt(sf.isNormalizedEst4), .fill = maybeUnitFill(4), .tol = exact },
    ...
    .{ .name = "LOADX", .impl = adapt(sf.loadX), .tol = exact },
    .{ .name = "LOAD1", .impl = adapt(sf.load1), .tol = exact },
    .{ .name = "LOADPTR", .impl = manual(1, 1, load.ptr), .tol = exact },
    .{ .name = "LOADPTRU", .impl = manual(1, 1, load.ptrU), .tol = exact },
    .{ .name = "LOADXPTRU", .impl = manual(1, 1, load.xPtrU), .tol = exact },
    .{ .name = "LOAD1PTRU", .impl = manual(1, 1, load.onePtrU), .tol = exact },
    .{ .name = "LOAD2PTRU", .impl = manual(1, 1, load.twoPtrU), .tol = exact },
    .{ .name = "LOAD3PTRU", .impl = manual(1, 1, load.threePtrU), .tol = exact },
    .{ .name = "GETX", .impl = adapt(sf.x), .tol = exact },
    .{ .name = "GETY", .impl = adapt(sf.y), .tol = exact },
    .{ .name = "GETZ", .impl = adapt(sf.z), .tol = exact },
    .{ .name = "GETW", .impl = adapt(sf.w), .tol = exact },
    .{ .name = "SETX", .impl = adapt(sf.withX), .tol = exact },
    .{ .name = "SETY", .impl = adapt(sf.withY), .tol = exact },
    .{ .name = "SETZ", .impl = adapt(sf.withZ), .tol = exact },
    .{ .name = "SETW", .impl = adapt(sf.withW), .tol = exact },
    .{ .name = "SETI", .impl = adapt(sf.withI), .fill = &setIndex, .tol = exact },
    .{ .name = "SPLATX", .impl = adapt(sf.splatX), .tol = exact },
    .{ .name = "SPLATY", .impl = adapt(sf.splatY), .tol = exact },
    .{ .name = "SPLATZ", .impl = adapt(sf.splatZ), .tol = exact },
    .{ .name = "SPLATW", .impl = adapt(sf.splatW), .tol = exact },
    .{ .name = "SWIZZLEWZYX", .impl = manual(1, 1, special.swizzleWZYX), .tol = exact },
    .{ .name = "STOREPTR", .impl = manual(2, 1, store.ptr), .tol = exact },
    .{ .name = "STORE1PTR", .impl = manual(2, 1, store.onePtr), .tol = exact },
    .{ .name = "STORE2PTR", .impl = manual(2, 1, store.twoPtr), .tol = exact },
    .{ .name = "STORE3PTR", .impl = manual(2, 1, store.threePtr), .tol = exact },
    .{ .name = "STOREPTRU", .impl = manual(2, 1, store.ptrU), .tol = exact },
    .{ .name = "STORE1PTRU", .impl = manual(2, 1, store.onePtrU), .tol = exact },
    .{ .name = "STORE2PTRU", .impl = manual(2, 1, store.twoPtrU), .tol = exact },
    .{ .name = "STORE3PTRU", .impl = manual(2, 1, store.threePtrU), .tol = exact },

    .{ .name = "TRANSPOSE4X1", .impl = adapt(sf.transpose4x1), .tol = exact },
    .{ .name = "TRANSPOSE1X4", .impl = adapt(sf.transpose1x4), .tol = exact },
    .{ .name = "TRANSPOSE4X2", .impl = adapt(sf.transpose4x2), .tol = exact },
    .{ .name = "TRANSPOSE2X4", .impl = adapt(sf.transpose2x4), .tol = exact },
    .{ .name = "TRANSPOSE4X3", .impl = adapt(sf.transpose4x3), .tol = exact },
    .{ .name = "TRANSPOSE3X4", .impl = adapt(sf.transpose3x4), .tol = exact },
    .{ .name = "TRANSPOSE4X4", .impl = adapt(sf.transpose4x4), .tol = exact },
    .{ .name = "TRANSPOSE16X16", .impl = adapt(sf.transpose16x16), .tol = exact },

    .{ .name = "QUATCONJUGATE", .impl = adapt(q.conjugate), .fill = &quats, .tol = exact },
    ...
    .{ .name = "MAT4TRANSPOSE", .impl = adapt(m4.transpose), .tol = exact },
    .{ .name = "MAT4INVERT", .impl = manual(4, 5, special.mat4Invert), .fill = &affine, .tol = composed, .defined = &.{ 4, 4, 4, 4, 1 } },
    .{ .name = "MAT4COLUMNMULTIPLY", .impl = adapt(m4.columnMultiply), .tol = exact },
    .{ .name = "MAT4SCALING", .impl = adapt(m4.scaling), .tol = exact },
    .{ .name = "MAT4TRANSLATE", .impl = adapt(m4.translate), .tol = rounding },
    .{ .name = "MAT4ISORTHOGONAL", .impl = adapt(m4.isOrthogonal), .fill = &maybeOrthogonal, .tol = exact },
    ...
    .{ .name = "MAT4ADD", .impl = adapt(m4.add), .tol = exact },
    .{ .name = "MAT4SUB", .impl = adapt(m4.sub), .tol = exact },

    .{ .name = "TRANSFORMMUL", .impl = adapt(math.transform.mul), .fill = &transforms, .tol = composed },

    .{ .name = "SOAFLOAT3ADD", .impl = adapt(s3.add), .tol = exact },
    .{ .name = "SOAFLOAT3SUB", .impl = adapt(s3.sub), .tol = exact },
    .{ .name = "SOAFLOAT3NEG", .impl = adapt(s3.neg), .tol = exact },
    .{ .name = "SOAFLOAT3MUL", .impl = adapt(s3.mul), .tol = exact },
    .{ .name = "SOAFLOAT3MULSCALAR", .impl = adapt(s3.mulScalar), .tol = exact },
    .{ .name = "SOAFLOAT3DIV", .impl = adapt(s3.div), .fill = &nonZero, .tol = exact },
    .{ .name = "SOAFLOAT3DIVSCALAR", .impl = adapt(s3.divScalar), .fill = &nonZero, .tol = exact },
    .{ .name = "SOAFLOAT3LT", .impl = adapt(s3.lt), .tol = exact },
    .{ .name = "SOAFLOAT3LE", .impl = adapt(s3.le), .tol = exact },
    .{ .name = "SOAFLOAT3GT", .impl = adapt(s3.gt), .tol = exact },
    .{ .name = "SOAFLOAT3GE", .impl = adapt(s3.ge), .tol = exact },
    .{ .name = "SOAFLOAT3EQ", .impl = adapt(s3.eq), .tol = exact },
    .{ .name = "SOAFLOAT3NE", .impl = adapt(s3.ne), .tol = exact },

    .{ .name = "SOAQUATNEG", .impl = adapt(sq.neg), .tol = exact },
    .{ .name = "SOAQUATCONJUGATE", .impl = adapt(sq.conjugate), .tol = exact },
    .{ .name = "SOAQUATADD", .impl = adapt(sq.add), .tol = exact },
    .{ .name = "SOAQUATMUL", .impl = adapt(sq.mul), .tol = rounding },
    .{ .name = "SOAQUATMULSCALAR", .impl = adapt(sq.mulScalar), .tol = exact },
    .{ .name = "SOAQUATDOT", .impl = adapt(sq.dot), .tol = rounding },
    .{ .name = "SOAQUATEQ", .impl = adapt(sq.eq), .tol = exact },

    .{ .name = "INTALLTRUE", .impl = adapt(si.all_true), .tol = exact },
    .{ .name = "INTALLFALSE", .impl = adapt(si.all_false), .tol = exact },
    .{ .name = "INTMASKSIGN", .impl = adapt(si.mask_sign), .tol = exact },
    .{ .name = "INTMASKSIGNXYZ", .impl = adapt(si.mask_sign_xyz), .tol = exact },
    .{ .name = "INTMASKSIGNW", .impl = adapt(si.mask_sign_w), .tol = exact },
    .{ .name = "INTMASKNOTSIGN", .impl = adapt(si.mask_not_sign), .tol = exact },
    .{ .name = "INTMASKFFFF", .impl = adapt(si.mask_ffff), .tol = exact },
    .{ .name = "INTMASK0000", .impl = adapt(si.mask_0000), .tol = exact },
    .{ .name = "INTMASKFFF0", .impl = adapt(si.mask_fff0), .tol = exact },
    .{ .name = "INTMASKF000", .impl = adapt(si.mask_f000), .tol = exact },
    .{ .name = "INTMASK0F00", .impl = adapt(si.mask_0f00), .tol = exact },
    .{ .name = "INTMASK00F0", .impl = adapt(si.mask_00f0), .tol = exact },
    .{ .name = "INTMASK000F", .impl = adapt(si.mask_000f), .tol = exact },

    .{ .name = "INTLOADX", .impl = adapt(si.loadX), .fill = &boolean, .tol = exact },
    .{ .name = "INTLOAD1", .impl = adapt(si.load1), .fill = &boolean, .tol = exact },
    .{ .name = "INTLOADPTR", .impl = manual(1, 1, loadInt.ptr), .fill = &bits, .tol = exact },
    .{ .name = "INTLOADPTRU", .impl = manual(1, 1, loadInt.ptrU), .fill = &bits, .tol = exact },
    .{ .name = "INTLOADXPTR", .impl = manual(1, 1, loadInt.xPtr), .fill = &bits, .tol = exact },
    .{ .name = "INTLOADXPTRU", .impl = manual(1, 1, loadInt.xPtrU), .fill = &bits, .tol = exact },
    .{ .name = "INTLOAD1PTR", .impl = manual(1, 1, loadInt.onePtr), .fill = &bits, .tol = exact },
    .{ .name = "INTLOAD1PTRU", .impl = manual(1, 1, loadInt.onePtrU), .fill = &bits, .tol = exact },
    .{ .name = "INTLOAD2PTR", .impl = manual(1, 1, loadInt.twoPtr), .fill = &bits, .tol = exact },
    .{ .name = "INTLOAD2PTRU", .impl = manual(1, 1, loadInt.twoPtrU), .fill = &bits, .tol = exact },
    .{ .name = "INTLOAD3PTR", .impl = manual(1, 1, loadInt.threePtr), .fill = &bits, .tol = exact },
    .{ .name = "INTLOAD3PTRU", .impl = manual(1, 1, loadInt.threePtrU), .fill = &bits, .tol = exact },
    .{ .name = "INTFROMFLOATROUND", .impl = adapt(si.fromFloatRound), .tol = exact },
    .{ .name = "INTFROMFLOATTRUNC", .impl = adapt(si.fromFloatTrunc), .tol = exact },

    .{ .name = "INTAND", .impl = adapt(si.@"and"), .fill = &bits, .tol = exact },
    .{ .name = "INTXOR", .impl = adapt(si.xor), .fill = &bits, .tol = exact },
    .{ .name = "INTNOT", .impl = adapt(si.not), .fill = &bits, .tol = exact },
    .{ .name = "INTANDNOT", .impl = adapt(si.andNot), .fill = &bits, .tol = exact },
    .{ .name = "INTCMPLT", .impl = adapt(si.cmpLt), .fill = &comparable, .tol = exact },
    .{ .name = "INTCMPLE", .impl = adapt(si.cmpLe), .fill = &comparable, .tol = exact },
    .{ .name = "INTCMPGT", .impl = adapt(si.cmpGt), .fill = &comparable, .tol = exact },
    .{ .name = "INTCMPGE", .impl = adapt(si.cmpGe), .fill = &comparable, .tol = exact },
    .{ .name = "INTCMPEQ", .impl = adapt(si.cmpEq), .fill = &comparable, .tol = exact },
    .{ .name = "INTCMPNE", .impl = adapt(si.cmpNe), .fill = &comparable, .tol = exact },
    .{ .name = "INTSIGN", .impl = adapt(si.sign), .tol = exact },
    .{ .name = "INTAREALLTRUE", .impl = adapt(si.areAllTrue), .fill = &laneMask, .tol = exact },
    .{ .name = "INTAREALLTRUE3", .impl = adapt(si.areAllTrue3), .fill = &laneMask, .tol = exact },
    .{ .name = "INTAREALLTRUE2", .impl = adapt(si.areAllTrue2), .fill = &laneMask, .tol = exact },
    .{ .name = "INTAREALLTRUE1", .impl = adapt(si.areAllTrue1), .fill = &laneMask, .tol = exact },
    .{ .name = "INTAREALLFALSE", .impl = adapt(si.areAllFalse), .fill = &laneMask, .tol = exact },
    .{ .name = "INTAREALLFALSE3", .impl = adapt(si.areAllFalse3), .fill = &laneMask, .tol = exact },
    .{ .name = "INTAREALLFALSE2", .impl = adapt(si.areAllFalse2), .fill = &laneMask, .tol = exact },
    .{ .name = "INTAREALLFALSE1", .impl = adapt(si.areAllFalse1), .fill = &laneMask, .tol = exact },
    .{ .name = "INTMOVEMASK", .impl = adapt(si.moveMask), .fill = &laneMask, .tol = exact },
    .{ .name = "INTSWIZZLEZWXY", .impl = manual(1, 1, loadInt.swizzleZWXY), .fill = &bits, .tol = exact },
    .{ .name = "INTSHIFTL", .impl = adapt(si.shiftL), .fill = &shift, .tol = exact },
    .{ .name = "INTSHIFTR", .impl = adapt(si.shiftR), .fill = &shift, .tol = exact },
    .{ .name = "INTSHIFTRU", .impl = adapt(si.shiftRu), .fill = &shift, .tol = exact },

    .{ .name = "ZERO", .impl = adapt(sf.zero), .tol = exact },
    .{ .name = "ONE", .impl = adapt(sf.one), .tol = exact },
    .{ .name = "XAXIS", .impl = adapt(sf.x_axis), .tol = exact },
    .{ .name = "YAXIS", .impl = adapt(sf.y_axis), .tol = exact },
    .{ .name = "ZAXIS", .impl = adapt(sf.z_axis), .tol = exact },
    .{ .name = "WAXIS", .impl = adapt(sf.w_axis), .tol = exact },
